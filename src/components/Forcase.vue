  ...
  <section class="wrap">
    <div class="mask">
      ...
      <section class="card" v-for="(item, key) in datas" :key="key">
        <header class="card-header">{{ item.lable }} {{ currentDate() }}</header>
        <div class="card-body">
          <div class="row row-header">
            <div class="row-name">站址</div>
          ...
          <div :class="`row ${(task.process !== 0 && task.process !== 100) && 'row-running'} ${task.process === 100  && 'row-done'} ${(task.beforeLeft >0 && task.beforeLeft < 120) && 'row-warning'}`" v-for="(task, index) in item.list" :key="index">
            ...
            <div class="row-process row-process-col">
              ...
              />
              <div v-if="task.processText" class="row-process-text">距开始: {{ task.processText }}</div>
            </div>
      ...
      map(news,({lable,list},k)=>{
        list.forEach(n => {
          ...
          if(st <= current && en >= current){
            ...
            left =  n.duration - his
            processText = ''
            process = Math.max(parseInt((his/n.duration) * 100),0.1)
          }
          n.process = process
